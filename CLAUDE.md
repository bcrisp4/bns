...
- **`serve` cobra flags minimal** — `--config/-c`, `--listen.udp`, `--listen.tcp`, `--upstream`, `--pprof`. `--blocklist` removed when HTTP source landed; use YAML. Everything else (log level, query log, cache capacity, blocklist sources) go via env (`BNS_*`, `__` for nesting) or YAML. Note: env vars cannot index slice config (see viper gotcha below).
